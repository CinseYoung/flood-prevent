<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">采砂标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/CS_CSDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '采砂对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__CS_CSDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/CS_CSDX_I.vue')
    },
    seeYjclAdd() {
