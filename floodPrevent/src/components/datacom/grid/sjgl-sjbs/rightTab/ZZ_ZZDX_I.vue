<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">闸站工程标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/ZZ_ZZDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '闸站工程对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__ZZ_ZZDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/ZZ_ZZDX_I.vue')
    },
    seeYjclAdd() {
