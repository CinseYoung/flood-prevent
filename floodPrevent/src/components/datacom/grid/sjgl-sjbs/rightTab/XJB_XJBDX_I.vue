<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">橡胶坝标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/XJB_XJBDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '橡胶坝对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__XJB_XJBDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/XJB_XJBDX_I.vue')
    },
    seeYjclAdd() {
