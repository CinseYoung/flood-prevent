<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">堤防标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/DF_DFDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '堤防对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__DF_DFDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/DF_DFDX_I.vue')
    },
    seeYjclAdd() {
