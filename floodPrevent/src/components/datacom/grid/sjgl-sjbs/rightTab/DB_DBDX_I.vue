<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">大坝标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/DB_DBDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '大坝对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__DB_DBDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/DB_DBDX_I.vue')
    },
    seeYjclAdd() {
