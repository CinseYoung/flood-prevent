<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">渠道标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/QD_QDDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '渠道对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__QD_QDDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/QD_QDDX_I.vue')
    },
    seeYjclAdd() {
