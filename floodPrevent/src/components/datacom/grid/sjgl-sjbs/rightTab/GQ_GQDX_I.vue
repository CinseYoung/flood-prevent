<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">灌区工程标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/GQ_GQDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '灌区对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__GQ_GQDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/GQ_GQDX_I.vue')
    },
    seeYjclAdd() {
