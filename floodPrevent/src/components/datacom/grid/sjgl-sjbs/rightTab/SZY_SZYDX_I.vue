<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">水资源分区标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/SZY_SZYDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '水资源分区对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__SZY_SZYDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/SZY_SZYDX_I.vue')
    },
    seeYjclAdd() {
