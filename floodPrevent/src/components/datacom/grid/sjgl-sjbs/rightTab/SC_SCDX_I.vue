<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">水厂标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/SC_SCDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '水厂对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__SC_SCDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/SC_SCDX_I.vue')
    },
    seeYjclAdd() {
