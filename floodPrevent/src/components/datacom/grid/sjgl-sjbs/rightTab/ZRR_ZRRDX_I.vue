<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">自然人标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/ZRR_ZRRDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '自然人对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__ZRR_ZRRDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/ZRR_ZRRDX_I.vue')
    },
    seeYjclAdd() {
