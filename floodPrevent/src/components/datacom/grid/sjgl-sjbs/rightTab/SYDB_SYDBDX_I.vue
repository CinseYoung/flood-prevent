<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">地表水水源地标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/SYDB_SYDBDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '地表水源地对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__SYDB_SYDBDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/SYDB_SYDBDX_I.vue')
    },
    seeYjclAdd() {
