<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">河流标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/HL_HLDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '河流对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__HL_HLDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/HL_HLDX_I.vue')
    },
    seeYjclAdd() {
