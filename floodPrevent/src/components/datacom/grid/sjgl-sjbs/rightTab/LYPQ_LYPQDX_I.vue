<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">流域片区标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/LYPQ_LYPQDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '流域片区对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__LYPQ_LYPQDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/LYPQ_LYPQDX_I.vue')
    },
    seeYjclAdd() {
