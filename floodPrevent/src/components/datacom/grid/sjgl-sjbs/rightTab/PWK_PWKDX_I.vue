<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">排污口标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/PWK_PWKDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '排污口对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__PWK_PWKDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/PWK_PWKDX_I.vue')
    },
    seeYjclAdd() {
