<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">流域标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/LY_LYDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '流域对象ID', propname: 'DXID' },
        { labelname: '水资源对象ID', propname: 'SZYDXID' },
        { labelname: '水土保对象ID', propname: 'STBDXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__LY_LYDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/LY_LYDX_I.vue')
    },
    seeYjclAdd() {
