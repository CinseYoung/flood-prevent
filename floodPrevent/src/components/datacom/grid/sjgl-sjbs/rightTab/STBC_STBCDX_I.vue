<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">水土保持区划标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/STBC_STBCDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '水土保持区划对象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__STBC_STBCDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/STBC_STBCDX_I.vue')
    },
    seeYjclAdd() {
