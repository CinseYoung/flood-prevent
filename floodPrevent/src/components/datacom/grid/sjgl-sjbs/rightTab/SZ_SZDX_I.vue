<template>
  <div>
    ...
    <div class="gr-datagrid">
      <div class="gr-tag">
        <span class="data-title">水闸标识</span>
...
<script>
import search from '@/components/datacom/search/sjgl-sjbs/SZ_SZDX_I.vue'
...
export default {
  data() {
    return {
      table_header: [
        { labelname: '对水闸象ID', propname: 'DXID' },
  ...
  methods: {
    load(param) {
      this.api.get(this, 'data_sjgl_sjbs__SZ_SZDX_I', this.result, param)
    },
    searchSubmit(data) {
      this.load(data)
    },
    ...
    seeYjcl(data) {
      ...
      this.dialogContentComponent = () => import('@/components/datacom/dialog/sjgl-sjbs/SZ_SZDX_I.vue')
    },
    seeYjclAdd() {
